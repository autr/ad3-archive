...
import { Vector3, Matrix, TmpVectors, CreateRibbonVertexData, Mesh, Path3D, _CreationDataStorage } from '@babylonjs/core'
...
export function CalculateOffset(current, previous, next, radius) {
	...
	const dotProduct = Vec3.Dot(vecToLast, vecToNext)
...
export function GenerateStrokePoints( points = [], radius, smooth, close, roundedEnds, sides ) {
	...
	for (let i = 0; i < length; i++) {
		...
		if (last && next) {
			...
			const math = Curve3.CreateQuadraticBezier( edgeA, current, edgeB, smooth + 1 )
			if (smooth == 0) {
...
export function CreateTubeVertexData(options) {
    ...
    const vertexData = CreateRibbonVertexData(ribbonOptions);
    return vertexData;
