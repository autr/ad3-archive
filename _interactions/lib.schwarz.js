function calculateBarycentricCoordinatesWithEdges(width, height, x, y, z) {

	// Ensure the barycentric coordinates sum to 1
	if (x + y + z !== 1) throw new Error("Barycentric coordinates must sum to 1.")
...
// Example usage
const result = calculateBarycentricCoordinatesWithEdges(100, 50, 0.2, 0.5, 0.3);
console.log(`Cartesian coordinates of the point: (${result.coordinates.join(', ')})`);
result.edges.forEach((edge, index) => {
	console.log(`Edge ${index + 1}: [(${edge[0].join(', ')}) to (${edge[1].join(', ')})]`);
});
