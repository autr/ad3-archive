...
function getsnub(p,q,r) {
	...
	function refine(f,s) {
		...
		// f(a) = -A(dx)
		// dx = -Invert(A)(f(a))
		var m = Invert(Jacobian(f,a,b,1e-6));
		var s = f(a,b);
		var dx = MSetArray(m,s);
...
export function Schwarz(angles) {
...
export function ApplyOperation(trans,s,P,Q,R) {
...
export function MakeMatrix(trans,P,Q,R) {
	var m0 = ApplyOperation(trans,[1,0,0],P,Q,R);
	var m1 = ApplyOperation(trans,[0,1,0],P,Q,R);
	var m2 = ApplyOperation(trans,[0,0,1],P,Q,R);
...
// Returns new array
export function MSetArray(trans,s,P,Q,R) {
	var res = new Array(s.length);
	for (var i = 0; i < s.length; i++) {
		res[i] = ApplyOperation(trans,s[i],P,Q,R);
	}
	return res;
}

export function ApplyMatrix(m,u) {
...
export function Apply4Matrix(m,v) {
	console.assert(m.length == 16);
